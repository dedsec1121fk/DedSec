...
```bash
pip install flask && pip install blessed && pip install flask-socketio && pip install werkzeug && pip install requests && pip install datetime && pip install geopy && pip install pydub
```
...
6. **dedsec_music_player.py**  
   - A terminal-based music player designed for **Termux** with an interactive **curses-based** menu.  
   - Automatically detects music folders and supports playback of `.mp3`, `.wav`, `.flac`, `.m4a`, `.ogg`, and `.aac` files.  
   - Provides **controls** including play, pause, next, previous, shuffle, and stop.  
   - Features a **menu-driven interface** for easy folder and track selection.
