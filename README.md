...
3)Michrophone phishing h4cks,that they create phishing michrophone websites so you can listen to conversations abou the  party.😉

4)Location h4ck,that it create a phishing location website so you can find the exact address of the party.😉
...
pkg upgrade -y  
```
Also copy the essential dependencies fron the last paragraph so you can clone after the repository.
...
### Run The Menu
After all the setup use the command ```cd DedSec/Scripts && python menu.py``` and this will setup a menu that will run everytime you open Termux so you can easily access all the programs.

Files are saved in named folders like Camera-Phish etc,in your internal storage in the Download folder,use a file explorer to access them,also in this folder you must add images camera.jpg and record.jpg to the login screens have background after the first use.
