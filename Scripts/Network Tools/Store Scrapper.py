...
def ensure_termux_storage():
    ...
    if shutil.which("termux-setup-storage"):
        try:
            print("📁 Enabling Termux storage access (Android may prompt permission once)...")
...
def main():
    ...
    deep = input("Enable per-product DEEP scrape (recommended)? (y/n): ").strip().lower().startswith("y")
    want_reviews = input("Extract reviews when available? (y/n): ").strip().lower().startswith("y")
    download_imgs = input("Download product images? (y/n): ").strip().lower().startswith("y")

    headless_allowed = input("Enable headless fallback when needed? (y/n): ").strip().lower().startswith("y")
    if headless_allowed and not headless_available():
        headless_warn_once()
        headless_allowed = False

    resume = input("Resume previous run if found? (y/n): ").strip().lower().startswith("y")
    domain_crawler_enabled = input("Enable DOMAIN CRAWLER MODE (site-wide discovery)? (y/n): ").strip().lower().startswith("y")
    ...
    if resume and st.get("queue"):
        ...
        print(f"\nResume: pending={len(queue)} processed={len(processed)} engine={engine_hint}\n")
    else:
