...
DEFAULT_EXCLUDE_REL_PREFIXES = [
    Path("home/storage"),
    Path("usr/var/cache/apt/archives"),
    Path("usr/var/log"),
    Path("usr/tmp"),
]
...
def make_backup(mode: str, include_caches: bool, compression_level: int, enable_home_manifest: bool):
    ...
    print("=== DedSec Backup v5 (No-Root) ===")
    print(f"Mode   : {mode}")
    ...
    try:
        ...
        if free < max(est * 0.30, 200 * 1024 * 1024):
            print("[!] Warning: Low free space.")
    except Exception:
        pass

    tmp = Path(".dedsec_backup_tmp")
    if tmp.exists():
    ...
    try:
        ...
        print(f"[!] Could not verify ZIP: {e}")
...
def restore_zip(mode: str):
    ...
    if mode == "dry":
        ...
        confirm = "RESTORE"
        print("!!! WARNING !!! Restore may overwrite files.")
...
def verify_home_manifest():
    ...
    with zipfile.ZipFile(DOWNLOADS_ZIP, "r") as zf:
        try:
            ...
            print("[!] No home manifest in backup. Enable it in config and re-backup.")
            return
    ...
    for line in manifest:
        ...
        try:
            ...
            bad += 1
    print(f"OK: {ok} | Changed: {bad} | Missing: {missing}")
...
def menu():
    ...
    while True:
        ...
        print("1) Backup using config")
        print("2) Restore FULL")
        print("3) Restore HOME-only")
        print("4) Dry LIST")
