...
# --- Configuration ---
RADIO_DIR = os.path.expanduser("~/Radio")
GIT_REPO = "https://github.com/dedsec1121fk/DedSec-Radio"
DEDSEC_RADIO_PATH = os.path.join(RADIO_DIR, "DedSec-Radio")
...
def get_stations():
    ...
    lst = []
    for item in os.listdir(RADIO_DIR):
        path = os.path.join(RADIO_DIR, item)
...
def play_station_music(stdscr):
    ...
    station_path = os.path.join(RADIO_DIR, stations[current_station_index])
...
def update_radio_stations(stdscr):
    ...
    stdscr.refresh()
    try:
        if os.path.isdir(os.path.join(DEDSEC_RADIO_PATH, ".git")):
