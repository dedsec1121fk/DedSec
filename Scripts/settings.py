...
Music Artists:BFR TEAM,PLANNO MAN,MOUTRO,VG,KouNouPi,ADDICTED,JAVASPA.ZY.A2.N,ICE,Lefka City,HXLX,Giannis Vardis,Lavyrinthos
...
def list_directory_entries(path, base_path):
    ...
    Directories are prefixed with "[DIR] " and a ".. (Go Back)" option is included if not at the base.
    """
    entries = []
    ...
    for entry in sorted(os.listdir(path)):
        ...
        if os.path.isdir(full):
            entries.append((f"[DIR] {entry}", full))
        elif entry.endswith(".py"):
...
def update_packages_modules():
    pip_command = "pip install flask && pip install blessed && pip install flask-socketio && pip install werkzeug && pip install requests && pip install datetime && pip install geopy && pip install pydub && pip install pycryptodome && pip install mutagen && pip install cryptography"
    termux_command = "termux-setup-storage && pkg update -y && pkg upgrade -y && pkg install python git fzf nodejs openssh nano jq wget unzip curl proot openssl aapt rust clouflared"
