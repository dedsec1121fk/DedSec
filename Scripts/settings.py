...
def update_dedsec():
    ...
    if existing_dedsec_path:
        stdout, stderr = run_command("git pull", cwd=existing_dedsec_path)
...
# ------------------------------
# New Combined Menu for Scripts (without folders)
# ------------------------------
...
def build_combined_menu(scripts):
    ...
    This version lists all scripts individually without categorizing them into folders.
    """
    menu_items = []
    for script in sorted(scripts, key=lambda s: format_script_name(s).lower()):
        menu_items.append((format_script_name(script), "script", script))
    # Return an empty dictionary in place of category_scripts since no folders are used.
    return menu_items, {}
...
def run_combined_list_menu():
    ...
    menu_items, _ = build_combined_menu(all_scripts)
