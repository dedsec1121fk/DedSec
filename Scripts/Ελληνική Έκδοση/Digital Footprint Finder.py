...
# --- ΕΛΕΓΧΟΣ ΕΞΑΡΤΗΣΕΩΝ ΑΝΘΕΚΤΙΚΟΣ ΣΕ ΣΦΑΛΜΑΤΑ ---
def check_and_install_deps():
    ...
    try:
        ...
        print("\n[ΚΑΤΑΣΤΑΣΗ] Εγκατάσταση απαιτούμενων εξαρτήσεων (requests, colorama, beautifulsoup4)...")
        try:
...
# ----------------------------------------------------------------------
# ΑΝΑΖΗΤΗΣΗ ΣΕ ΜΗΧΑΝΕΣ ΑΝΑΖΗΤΗΣΗΣ (DuckDuckGo HTML)
# ----------------------------------------------------------------------

def dork_search(username):
    print(f"\n{Fore.YELLOW}[*] Εκτέλεση αναζήτησης για '{username}'...{Style.RESET_ALL}")
    query = f'"{username}"' 
...
def analyze_site(site_name, url_template, username, verbose=False):
    ...
    try:
        ...
        if verbose: print(f"{Fore.RED}Σφάλμα κατά τον έλεγχο {site_name}: {e}{Style.RESET_ALL}")
        return None
...
def main():
    ...
    print(f"\n{Fore.CYAN}Εύρεση Ψηφιακών Ιχνών v9 (Ελληνική Έκδοση){Style.RESET_ALL}")
    ...
    # Έλεγχοι API
    print(f"\n{Fore.BLUE}[*] Έλεγχος APIs...{Style.RESET_ALL}")
    ...
    # Σάρωση Ιστοτόπων
    print(f"\n{Fore.BLUE}[*] Σάρωση Πλατφορμών...{Style.RESET_ALL}")
    ...
    # --- ΕΝΗΜΕΡΩΜΕΝΗ ΛΟΓΙΚΗ ΑΠΟΘΗΚΕΥΣΗΣ ---
    # 1. Προσδιορισμός βασικού φακέλου Λήψεων για Termux
    base_dir = os.path.join(os.path.expanduser('~'), 'storage', 'downloads')
    
    # Εφεδρικός έλεγχος εάν ο χώρος αποθήκευσης δεν έχει αντιστοιχηθεί σωστά
    if not os.path.exists(base_dir):
        if os.path.exists("/sdcard/Download"):
             base_dir = "/sdcard/Download"
        else:
             base_dir = os.path.expanduser('~') # Τελευταία λύση: Αρχικός φάκελος

    # 2. Δημιουργία φακέλου 'Ψηφιακά Ίχνη'
    save_dir = os.path.join(base_dir, 'Ψηφιακά Ίχνη')
    try:
        os.makedirs(save_dir, exist_ok=True)
    except Exception as e:
        print(f"{Fore.RED}[!] Σφάλμα δημιουργίας φακέλου {save_dir}: {e}")
        # Εφεδρική λύση: τρέχων κατάλογος εάν αρνηθεί η άδεια
        save_dir = "." 
    
    # 3. Ορισμός ονόματος αρχείου ως username.txt
    filename = os.path.join(save_dir, f"{username}.txt")
    
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(f"Αναφορά Ιχνών για: {username}\n")
        f.write("="*40 + "\n\n")
        f.write("ΑΜΕΣΕΣ ΤΑΥΤΟΠΟΙΗΣΕΙΣ:\n")
    ...
    print(f"Αρχείο αποθηκεύτηκε: {Fore.CYAN}{filename}{Style.RESET_ALL}")
    print("="*60)
