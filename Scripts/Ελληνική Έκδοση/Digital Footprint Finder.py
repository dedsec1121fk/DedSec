...
# ----------------------------------------------------------------------
# Μετάφραση σε Ελληνικά για όλα τα μηνύματα προς τον χρήστη.
# ----------------------------------------------------------------------
...
# --- CRASH-PROOF DEPENDENCY CHECK (ΕΛΕΓΧΟΣ ΕΞΑΡΤΗΣΕΩΝ) ---

def check_and_install_deps():
    """
    Checks if necessary modules are available. If not, installs them using pip.
    (Ελέγχει αν είναι διαθέσιμες οι απαραίτητες ενότητες. Αν όχι, τις εγκαθιστά χρησιμοποιώντας το pip.)
    """
    ...
    print("\n[STATUS] Τα απαιτούμενα πακέτα Python δεν βρέθηκαν.")
    print("[STATUS] Προσπαθώ να τα εγκαταστήσω τώρα χρησιμοποιώντας το pip...\n")
    
    try:
        cmd = [sys.executable, "-m", "pip", "install"] + required_packages
        # Εκτελεί την εντολή και ελέγχει για σφάλματα
        process = subprocess.run(cmd, check=True, capture_output=True, text=True)
        
        print("\n[SUCCESS] Η εγκατάσταση ολοκληρώθηκε.")
        print("[STATUS] Επανεκκίνηση του script για φόρτωση των νέων πακέτων...")
        
        # Restart the script
        os.execv(sys.executable, ['python'] + sys.argv)
        
    except subprocess.CalledProcessError as e:
        print(f"\n[ΚΡΙΣΙΜΟ ΣΦΑΛΜΑ] Αποτυχία εγκατάστασης εξαρτήσεων μέσω pip.")
        print("Παρακαλώ βεβαιωθείτε ότι το pip είναι εγκατεστημένο (pkg install python) και εκτελέστε τα ακόλουθα χειροκίνητα:")
        print(f"pip install {' '.join(required_packages)}")
        print(f"Λεπτομέρειες:\n{e.stderr}")
        sys.exit(1)
    except Exception as e:
        print(f"\n[ΚΡΙΣΙΜΟ ΣΦΑΛΜΑ] Προέκυψε ένα απροσδόκητο σφάλμα κατά την εγκατάσταση: {e}")
        sys.exit(1)
...
try:
    ...
    print("\n[ΘΑΝΑΣΙΜΟ ΣΦΑΛΜΑ] Οι εξαρτήσεις απέτυχαν να φορτωθούν μετά την προσπάθεια εγκατάστασης. Έξοδος.")
    sys.exit(1)


# --- REVISED MASSIVE INTEGRATED WEBSITE CONFIGURATION (250+ Sites) ---
# ΑΥΤΟ ΤΟ ΛΕΞΙΚΟ ΠΡΕΠΕΙ ΝΑ ΠΑΡΑΜΕΙΝΕΙ ΣΤΑ ΑΓΓΛΙΚΑ ΓΙΑ ΝΑ ΛΕΙΤΟΥΡΓΕΙ ΤΟ ΣΚΡΙΠΤ
...
def check_site(username, site_name, site_info, verbose=False):
    ...
    (Εκτελεί ένα αίτημα για έλεγχο ιστοσελίδας, συμπεριλαμβανομένης της εκθετικής αναμονής για όρια ρυθμού.)
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if verbose:
                # DEBUG OUTPUT: Μεταφρασμένο
                print(f"{Fore.BLUE}DEBUG:{Style.RESET_ALL} {site_name}: Προσπάθεια {attempt+1}/{max_retries}, Κατάσταση={response.status_code}, URL={response.url}")

            # --- Rate Limit Handling (Exponential Backoff) ---
            if response.status_code == 429:
                wait_time = base_wait_time * (2 ** attempt)
                if attempt < max_retries - 1:
                    # WARNING OUTPUT: Μεταφρασμένο
                    print(f"{Fore.YELLOW}ΠΡΟΕΙΔΟΠΟΙΗΣΗ:{Style.RESET_ALL} {site_name} έφτασε το όριο ρυθμού (429). Αναμονή {wait_time}s...")
                    time.sleep(wait_time)
                    continue
                else:
                    return (site_name, f"{Fore.MAGENTA}[ΑΠΟΤΥΧΙΑ ΟΡΙΟΥ ΡΥΘΜΟΥ]{Style.RESET_ALL}", "N/A")

            # --- Detection Logic ---
            
            # Case 1: Explicit 'Not Found' Code (e.g., 404)
            if response.status_code == not_found_code:
                return (site_name, f"{Fore.RED}[ΔΕΝ ΒΡΕΘΗΚΕ]{Style.RESET_ALL}", "N/A")
            ...
            if response.status_code in [200, 301, 302, 307]:
                if not not_found_text:
                    return (site_name, f"{Fore.GREEN}[ΒΡΕΘΗΚΕ]{Style.RESET_ALL}", response.url)
            ...
            # Case 4: Other hard errors (e.g., 403 Forbidden, 400 Bad Request)
                ...
                if response.status_code in [403, 400]:
                    status_text = f"{status_color}[ΑΠΟΚΛΕΙΣΜΕΝΟ/ΣΦΑΛΜΑ]{Style.RESET_ALL}"
                else:
                    status_text = f"{status_color}[ΥΠΟΠΤΟ ({response.status_code})]{Style.RESET_ALL}"
                return (site_name, status_text, response.url)

        except requests.exceptions.Timeout:
            return (site_name, f"{Fore.MAGENTA}[ΧΡΟΝΟΛΗΞΗ]{Style.RESET_ALL}", "N/A")
        except requests.exceptions.SSLError:
            return (site_name, f"{Fore.MAGENTA}[ΣΦΑΛΜΑ SSL]{Style.RESET_ALL}", "N/A")
        except requests.exceptions.ConnectionError:
            return (site_name, f"{Fore.MAGENTA}[ΣΦΑΛΜΑ ΣΥΝΔΕΣΗΣ]{Style.RESET_ALL}", "N/A")
        except Exception as e:
            if verbose:
                # EXCEPTION OUTPUT: Μεταφρασμένο
                print(f"{Fore.RED}ΕΞΑΙΡΕΣΗ:{Style.RESET_ALL} {site_name}: {e}")
            return (site_name, f"{Fore.MAGENTA}[ΑΓΝΩΣΤΟ ΣΦΑΛΜΑ]{Style.RESET_ALL}", "N/A")

    return (site_name, f"{Fore.MAGENTA}[ΜΕΓΙΣΤΕΣ ΕΠΑΝΑΛΗΨΕΙΣ]{Style.RESET_ALL}", "N/A")


def save_results_to_file(username, found_profiles):
    """Saves the results to the specified file path in Termux internal storage."""
    # (Αποθηκεύει τα αποτελέσματα στο καθορισμένο μονοπάτι αρχείου.)
    ...
    output_filename = f"{username}_footprint.txt"
    ...
    try:
        ...
        with open(output_path, 'w', encoding='utf-8') as f: # Χρησιμοποιούμε utf-8 για Ελληνικούς χαρακτήρες
            f.write(f"Αποτελέσματα 'Digital Footprint Finder' για: {username}\n")
            f.write(f"Ώρα Σάρωσης: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}\n")
            f.write("-" * 50 + "\n")
            
            if found_profiles:
                f.write("Βρέθηκαν Προφίλ:\n")
                # Sort profiles by site name for cleaner output
                for site, url in sorted(found_profiles):
                    f.write(f"  {site.ljust(25)}: {url}\n")
            else:
                f.write("Δεν βρέθηκαν οριστικά προφίλ στις ιστοσελίδες που ελέγχθηκαν.\n")

        print("-" * 60)
        print(f"{Fore.GREEN}ΕΠΙΤΥΧΙΑ:{Style.RESET_ALL} Τα αποτελέσματα αποθηκεύτηκαν στο:")
        print(f"{Fore.CYAN}{output_path}{Style.RESET_ALL}")
    
    except Exception as e:
        print(f"{Fore.RED}ΣΦΑΛΜΑ:{Style.RESET_ALL} Αδυναμία αποθήκευσης των αποτελεσμάτων σε αρχείο.")
...
def find_digital_footprint(username, max_workers, verbose):
    ...
    (Χρησιμοποιεί ThreadPoolExecutor για ταυτόχρονη αναζήτηση όλων των ιστοσελίδων και συλλογή αποτελεσμάτων.)
    """
    total_sites = len(SITES)
    
    print("\n" + "=" * 60)
    print(f"| {Fore.CYAN}ΣΤΟΧΟΣ:{Style.RESET_ALL} {Fore.WHITE}{username.ljust(50)} |")
    print(f"| {Fore.CYAN}ΕΛΕΓΧΘΕΙΣΕΣ ΙΣΤΟΣΕΛΙΔΕΣ:{Style.RESET_ALL} {Fore.WHITE}{str(total_sites).ljust(41)} |")
    print("=" * 60)
    
    # Header for the results table (Κεφαλίδα πίνακα αποτελεσμάτων)
    print(f"{Fore.YELLOW}{'ΠΛΑΤΦΟΡΜΑ'.ljust(15)} | {'ΚΑΤΑΣΤΑΣΗ'.ljust(25)} | {'URL'}{Style.RESET_ALL}")
    print("-" * 60)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_site):
            try:
                ...
                if raw_status in ["[ΒΡΕΘΗΚΕ]", "[ΔΕΝ ΒΡΕΘΗΚΕ]", "[ΑΠΟΚΛΕΙΣΜΕΝΟ/ΣΦΑΛΜΑ]", "[ΧΡΟΝΟΛΗΞΗ]", "[ΣΦΑΛΜΑ SSL]", "[ΣΦΑΛΜΑ ΣΥΝΔΕΣΗΣ]", "[ΑΓΝΩΣΤΟ ΣΦΑΛΜΑ]", "[ΑΠΟΤΥΧΙΑ ΟΡΙΟΥ ΡΥΘΜΟΥ]", "[ΜΕΓΙΣΤΕΣ ΕΠΑΝΑΛΗΨΕΙΣ]"]:
                    results.append(raw_status)
                
                # Collect found profiles for file output
                if raw_status == "[ΒΡΕΘΗΚΕ]":
                ...
                # ERROR OUTPUT: Μεταφρασμένο
                print(f"{Fore.RED}ΣΦΑΛΜΑ:{Style.RESET_ALL} Ο έλεγχος ιστοσελίδας δημιούργησε μια μη αναμενόμενη εξαίρεση: {exc}")

    end_time = time.time()
    
    # --- Final Summary (Τελική Περίληψη) ---
    found_count = len(found_profiles_for_file)
    
    print("-" * 60)
    print(f"{Fore.CYAN}ΠΕΡΙΛΗΨΗ:")
    print(f"{Fore.GREEN}  {found_count} προφίλ βρέθηκαν.")
    print(f"{Fore.WHITE}  Σύνολο ελεγχθεισών ιστοσελίδων: {total_sites}")
    print(f"{Fore.WHITE}  Συνολικός χρόνος: {end_time - start_time:.2f} δευτερόλεπτα.")
    print("-" * 60)
...
def main():
    """Handles argument parsing and execution. (Χειρίζεται την ανάλυση ορισμάτων και την εκτέλεση.)"""
    
    parser = argparse.ArgumentParser(description="Digital Footprint Finder (Εργαλείο OSINT).")
    parser.add_argument("username", nargs='?', help="Το όνομα χρήστη προς αναζήτηση.")
    parser.add_argument("-v", "--verbose", action="store_true", help="Ενεργοποίηση λεπτομερούς/εντοπισμού σφαλμάτων εξόδου.")
    parser.add_argument("-w", "--workers", type=int, default=20, help="Αριθμός ταυτόχρονων εργατών (threads). Προεπιλογή: 20.")
    args = parser.parse_args()
    
    # Simple banner (Απλό banner)
    print(f"\n{Fore.YELLOW}Digital Footprint Finder v7.1 - {Fore.RED}{len(SITES)} Πραγματικές Ιστοσελίδες & Προστασία από Crash{Style.RESET_ALL}\n")
    
    # If username is not provided via arguments, prompt the user
    if not args.username:
        username = input(f"{Fore.GREEN}Εισάγετε το όνομα χρήστη προς αναζήτηση: {Fore.CYAN}").strip()
    else:
        username = args.username.strip()
    
    if not username:
        print(f"{Fore.RED}Σφάλμα: Το όνομα χρήστη δεν μπορεί να είναι κενό. Έξοδος.")
        sys.exit(1)
        
    # Check the actual count of sites to ensure it's >= 250
    if len(SITES) < 250:
        print(f"{Fore.YELLOW}Προειδοποίηση: Φορτώθηκαν μόνο {len(SITES)} ιστοσελίδες. Ο στόχος ήταν 250.{Style.RESET_ALL}")
